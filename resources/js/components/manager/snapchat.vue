...
<template>
<div>
  <button type="button" class="btn btn-primary float-left" data-toggle="modal" data-target="#snap">
     Snapchat
  </button>
    <div class="modal fade" id="snap" ref="snap" tabindex="-1" aria-labelledby="personneInfoLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="personneInfoLabel">Snapchat</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form  @submit="addSnapchat" enctype="multipart/form-data">
        <div class="form-row">
...
export default{
    ...
    methods:{
      async addSnapchat(e){
        ...
        const res=await this.callApi('post','/api/personneTypeSnapchat',this.data3)
        if(res.status==200){
          this.s('Success')
        }else{
          this.swr()
